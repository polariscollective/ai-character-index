...
## Purpose
The root holds the project's two entry documents (`PLAN.md`, `README.md`) and the Next.js application: `app/` for the reader's four routes, the public MCP endpoint, the public proposal route and the admin portal, `next.config.mjs`, and a pnpm setup carrying Next, React, Auth.js and the MCP server libraries (plus `playwright-core` for the engine's browser checks).

`app/` has three parts sharing `app/lib/`. The reader's routes serve payloads to a static reader and are open, and so is `/api/mcp`, which only reads. `/api/submit` is open too and writes, which is why its library is mostly about what it refuses. The portal is rendered React behind a Google door and an allow-list, and holds every write that changes what the index says. The public pages stay plain files served from `public/` (a document reader and a form must keep working as plain files), while the portal is an application with a session, which is why it is the framework's.
...
| `README.md` | Front door: what the index is and holds, how it fits together, running it with and without credentials, proposing, the portal, judging, citations, checks, repo map, contributors and licence |
| `package.json` | Root package `ai-character-index` (private); `packageManager: pnpm@11.26.0`; `engines.node >=22.13 <25`; scripts: `dev`/`build`/`start` for Next, `test:routes` (every suite under `app/lib/__tests__/`), and the `predev`/`prebuild` pair that copies `site/` into the gitignored `public/`; deps: `next`, `react`, `react-dom`, `next-auth` (v5 beta, the version Auth.js ships for the App Router), and `@modelcontextprotocol/server`, `mcp-handler` and `zod`; devDep: `playwright-core ^1.61.1` |
...
- `README.md`'s repo-map table links `app/`, `engine/`, `engine/local_run.py`, `engine/spec-cite/cite.py`, `site/`, `methodology/`, `specs/` and `docs/superpowers/`, all present; it points onward to `CLAUDE.md` for the divergences from upstream and to `SYSTEM.md` for the system map.

## Dependency map
```mermaid
graph LR
  PJ[package.json] -->|packageManager pnpm@11.26.0| AS[pnpm/action-setup]
...
- PLAN.md §5's CI/CD table promises `ci.yml`, `notion-sync.yml`, `spec-watch.yml` alongside a deploy workflow; what exists is `ci.yml` and `deploy-runner.yml`, which builds the judging job's image. The published data is checked by `verify_supabase_provenance.py --publication=<uuid>` when a publication is built, not by a scheduled workflow. The site's deployment is Vercel's, fired by a push rather than by a workflow here.
