...
- **The panel is the index's one panel**, `frontier_fast`, read from `engine/panel/panel-config.json` as `display.panel`. No portal surface offers a panel choice. The command line keeps its `--panel` flag, as `compose_run.py` has: comparing one cell under two panels is work to expect later, and only its cost rules it out today.
