...
- `how-it-works.html` — what the index is, how to run it yourself, and the two
  proposal forms. Served at `/how-it-works` by a rewrite in `next.config.mjs`;
  `methodology.html` and `propose.html` are redirects to it, kept because links
  to both are already shared.
