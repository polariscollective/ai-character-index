/* The coverage board, at /coverage: how each lab's constitution scores,
 * behaviour by behaviour and as a document.
 *
 * It led the overview until 23 September 2026, when the front page took a board
 * built from a written file instead. It kept its route rather than being
 * deleted, because the figures it carries open on the passages behind them and
 * nothing else on the site does that.
 *
 ...
 * pressed.
 *
 * The board is site/board.js, which the front page's two boards draw too. What
 * is on it depends on the publication: out of ten it carries a final score out of 20,
...
function aboutFinal(content) {
  ...
  ["Rank", "Company", "Behaviours", "Whole document", "Final score"].forEach(name => {
  ...
  content.append(table, element("p", "", "Companies are ranked by this score. Companies level "
    + "on it share a place and the next rank skips. A company with no constitution has no "
    + "whole-document total to add, so it has no final score and comes after, unranked."));
}
...
async function initialize() {
  ...
  renderMethod();
  renderTies();
  board.wirePopover([document.querySelector(".matrix-wrap")]);
