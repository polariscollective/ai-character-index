...
| `overview.html`, `overview.js`, `governance.js`, `governance.json` | The front page, served at `/` and `/overview`: two views of one board (`board.js`) behind tabs. The first reads the current publication: the final score out of 20 at the top, the document as a whole out of 10 opening into its five criteria out of 2, and each behaviour category opening into its behaviours, every figure on the depth scale the publication carries. A publication out of four carries no assessment, so it shows the categories alone, out of 4. The contradictions of a document open in the sheet; everything else opens in a popover beside what was pressed. The second view, `?view=governance`, is the board of how each lab governs its model spec: the nine labs across and their scores down, each of the four questions opening into its checks. Its scores and text are editorial, from a research note of 18 September 2026 in its second pass, and live in `governance.json` rather than in the database. |
