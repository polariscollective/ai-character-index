...
| `overview.html`, `overview.js`, `governance.js`, `governance.json` | The front page, served at `/` and `/overview`, in two views behind tabs. The first is the grid of how deeply each specification covers each behaviour, read from the current publication. The second, `?view=governance`, is a board of how each lab governs its rulebook: a heat map of six labs on four questions and their ten checks, with a panel that shows the evidence behind any score. Its scores and text are editorial, from a research note of 18 September 2026, and live in `governance.json` rather than in the database. |
