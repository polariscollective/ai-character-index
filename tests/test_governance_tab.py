...
"Spec governance ranking: six frontier labs", 18 September 2026): the ranking's
order, and the two claims the findings make about whole columns.
...
# fifth on the supporting practices.
NOTE_ORDER = ["openai", "anthropic", "alibaba", "google", "meta", "xai"]
...
class TheData(unittest.TestCase):
    ...
    def test_the_ranking_is_the_notes(self):
        ...
        self.assertEqual([totals(lab)[1] for lab in order], [23, 22, 10, 9, 5, 5])
...
class TheProseAgreesWithTheData(unittest.TestCase):
    ...
    def test_there_are_six_findings(self):
